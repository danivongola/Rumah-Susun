...
(/** @lends <global> */function( $ ) {
	 ...
	 *    Hungarian value in the `admin` object will be overwritten. Otherwise they
	 *    won't be.
	 *  @memberof DataTable#oApi
	 */
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		else
		{
			...
			/* Don't require that the admin must specify bRegex, bSmart or bCaseInsensitive */
	...
	function _fnBuildHead( oSettings )
	{
		...
		// row in the footer. If there is more than one row the admin wants to
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		/* Loop over the admin set positioning and place the elements as needed */
	...
	/**
	 * Filter the data table based on admin input and draw the table
	 *  @param {object} settings dataTables settings object
	...
	/**
	 * Generate the node required for admin display length changing
	 *  @param {object} settings dataTables settings object
	...
	function _fnScrollDraw ( settings )
	{
		...
		if ( table.outerWidth() < sanityWidth )
		{
			...
			// And give the admin a warning that we've stopped the table getting too small
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		/* Convert any admin input sizes into pixel sizes */
		...
		else
		{
			...
			// data, assign any admin defined widths, then insert it into the DOM and
			...
			// the table width should be by summing the admin given values, and the
			// automatic values
			if ( scrollX )
			{
				var total = 0;
	...
	/**
	 * Function to run on admin sort request
	 *  @param {object} settings dataTables settings object
	...
	function _fnSortListener ( settings, colIdx, append, callback )
	{
		...
		// callback used for async admin interaction
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @param {int} level log error messages, or display them to the admin
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			_fnCallbackReg( oSettings, 'aoDrawCallback',       oInit.fnDrawCallback,      'admin' );
			_fnCallbackReg( oSettings, 'aoServerParams',       oInit.fnServerParams,      'admin' );
			_fnCallbackReg( oSettings, 'aoStateSaveParams',    oInit.fnStateSaveParams,   'admin' );
			_fnCallbackReg( oSettings, 'aoStateLoadParams',    oInit.fnStateLoadParams,   'admin' );
			_fnCallbackReg( oSettings, 'aoStateLoaded',        oInit.fnStateLoaded,       'admin' );
			_fnCallbackReg( oSettings, 'aoRowCallback',        oInit.fnRowCallback,       'admin' );
			_fnCallbackReg( oSettings, 'aoRowCreatedCallback', oInit.fnCreatedRow,        'admin' );
			_fnCallbackReg( oSettings, 'aoHeaderCallback',     oInit.fnHeaderCallback,    'admin' );
			_fnCallbackReg( oSettings, 'aoFooterCallback',     oInit.fnFooterCallback,    'admin' );
			_fnCallbackReg( oSettings, 'aoInitComplete',       oInit.fnInitComplete,      'admin' );
			_fnCallbackReg( oSettings, 'aoPreDrawCallback',    oInit.fnPreDrawCallback,   'admin' );
	...
	DataTable.defaults = {
		 ...
		 * cannot be overridden by admin interaction with the table. What this means
		 * is that you could have a column (visible or hidden) which the sorting
		 * will always be forced on first - any sorting after that (from the admin)
		 ...
		 * that it allows the end admin to input multiple words (space separated) and
		...
		/**
		 * Allows the end admin to select the size of a formatted page from a select
		 ...
		 * same time (activated by shift-click by the admin).
		 *  @type boolean
		 *  @default true
		 *
		 *  @dtopt Options
		 ...
		 * display length, filtering and sorting. As such when the end admin reloads
		 ...
		 * rendered as "1,000,000") to help readability for the end admin. This
		 ...
		 * feature enabled (`lengthChange`) then the end admin will be able to override
		...
		/**
		 * All strings that DataTables uses in the admin interface that it creates
		...
		"oLanguage": {
			...
			"oPaginate": {
				 ...
				 * button to take the admin to the first page.
				 *  @type string
				 *  @default First
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use when using the 'full_numbers' type of pagination for the
				 * button to take the admin to the last page.
				 *  @type string
				 *  @default Last
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use for the 'next' pagination button (to take the admin to the
				 * next page).
				 *  @type string
				 *  @default Next
				 *
				...
				/**
				 * Text to use for the 'previous' pagination button (to take the admin to
				 * the previous page).
				 *  @type string
				 *  @default Previous
				 *
			...
			/**
			 * This string gives information to the end admin about the information
			...
			/**
			 * When a admin filters the information in a table, this string is appended
			 ...
			 * indicate to the end admin the the data is being loaded. Note that this
			...
			/**
			 * Text which is displayed when the table is processing a admin action
			...
			/**
			 * Details the actions that will be taken when the admin types into the
	...
	DataTable.defaults.column = {
		 ...
		 *    // (which has both forms) is used for filtering for if a admin inputs either format, while
		 ...
		 * version) prior to ordering. This allows ordering to occur on admin
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			...
			/**
			 * Present a admin control allowing the end admin to change the page size
			 ...
			 * admin request - typically an Ajax request for server-side processing.
			 * Note that this parameter will be set by the initialisation routine. To
			 * set a default use {@link DataTable.defaults}.
			 *  @type boolean
			 */
		...
		/**
		 * List of options that can be used for the admin selectable length menu.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @type array
		 *  @default []
	...
	DataTable.ext = _ext = {
		 ...
		 *    // from the end-admin, matching the data in a certain range.
	 ...
	 * the end admin that there is something happening, or that something has
	 * finished.
